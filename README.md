...
**The short answer:** 100% accuracy at all noise levels. But more importantly — we found out *exactly why*, and it changes how we design the real thesis experiment.
...
[2] Normalise: divide by max(|IQ|)  ← NO mean subtraction (DC offset preserved)
     │
     ▼
[3] Reshape into (N × 1024 × 2) segments  ← I and Q as separate channels
     │
     ▼
...
|---|---|---|---|---|---|
| 3288FAD | 0.769330 | 0.000540 | ✅ | ✅ | ✅ |
| 3288FF2 | 0.612370 | 0.001482 | ✅ | ✅ | ✅ |
...
Each radio transmits a 10 kHz cosine wave. The I channel (blue) and Q channel (red) are 90° apart — this is the fundamental IQ representation. Even at this raw level, the two devices produce slightly different signals due to hardware impairments.
...
We designed a lightweight 1D Convolutional Neural Network with **52,258 trainable parameters** — roughly 80× smaller than typical deep learning models.

```
Input: (1024 samples × 2 channels)
  │
...
The standard deviation of 0.67 Hz means the CNN can rely on this feature being consistent across the entire recording, across different time windows, and (importantly) between training and test sets.
...
| Amplitude mean | 0.802352 | 0.836747 | 0.034 | ⚠️ Weak secondary |
| Phase std dev | 1.814 | 1.814 | 0.000 | ❌ No — identical |

### The CFO fingerprint explained

Every radio has a crystal oscillator that controls its operating frequency. No crystal is perfect — each one has a tiny manufacturing-specific error that shifts the transmitted signal to a slightly different frequency. This shift is called the **Carrier Frequency Offset (CFO)**.
...
The CNN answers the question: *"Is the tone closer to 9,418 Hz or 10,580 Hz?"* — which it can determine from just a few samples at the start of each segment (confirmed by saliency maps, which peak at sample 1–4 of 1,024).

> **Note on this experiment's limitation:** The 1,161 Hz separation is the *combined* oscillator error of both TX and RX hardware pairs. In the real thesis experiment (fixed receiver), only the TX oscillator changes between classes. The separation will be smaller — perhaps 50–200 Hz — making the problem harder and more scientifically rigorous.
...
> **Why this is meaningful:** In a real-world deployment, a 0 dB SNR corresponds to a very poor radio link — low transmission power, long range, or heavy interference. The fact that the CFO fingerprint survives this level of noise degradation suggests it will remain useful in challenging deployment conditions.
...
**Right** — Effect of noise on one segment: Shows visually how the I channel degrades from 20 dB → 10 dB → 0 dB SNR. Even at 0 dB the underlying tone frequency is preserved — which is why the CNN can still extract the CFO fingerprint.
...
The 1,161 Hz separation is consistent to within 0.67 Hz across the entire recording. It survives all three noise levels including 0 dB SNR. The CNN needed only 5–9 epochs to learn it. This is a real, physical, hardware-level impairment — not a dataset artifact.
...
Zero performance gap between random and temporal train/test splits across all three SNR levels. The fingerprint is time-invariant within a session — the CNN generalises to unseen time windows, not just unseen shuffled segments.

### ✅ Lightweight CNN architecture validated

52,258 parameters, AveragePooling1D, GlobalAveragePooling1D — converges cleanly in under 10 epochs. The architecture is confirmed for use in the thesis experiment.

### ⚠️ Both TX and RX changed between classes
...
MaxPooling keeps only the highest-amplitude feature in each window, which is great for image classification (sharp edges). For RF signals, subtle phase and timing patterns matter as much as amplitude peaks. AveragePooling preserves these by considering the full window.

**Why GlobalAveragePooling1D instead of Flatten?**  
After three conv layers, the tensor is shape (128, 128). Flattening gives 16,384 features going into the Dense layer — about 4 million parameters in that connection alone. GlobalAveragePooling1D reduces this to 128 features → ~16K parameters. Same representational power, 250× fewer weights to overfit.

**Why 0.0001 learning rate?**  
RF fingerprints are subtle. A large learning rate (e.g. 0.001) causes the optimizer to overshoot the narrow loss minima where the hardware impairment features live. The slower learning rate found in the Gemini session was the fix that broke the initial 50% flatline.
